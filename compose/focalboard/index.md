...
- *(El puerto `5432` de PostgreSQL no está expuesto al host, manteniéndose accesible únicamente dentro de la red interna de Docker por motivos de seguridad).*
...
> **Seguridad:** Se recomienda sustituir `focalboard_password` por una contraseña segura tanto en la cadena de conexión (`FB_DBCONNSTR`) como en `POSTGRES_PASSWORD` antes de desplegar en entornos compartidos o de producción.
...
- **Aislamiento de la base de datos**: Al no exponer el puerto `5432` hacia el exterior, la base de datos queda protegida frente a accesos indebidos desde la red local o internet.
